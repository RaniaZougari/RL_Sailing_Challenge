...
class QLearningAgent(BaseAgent):
    ...
    def act(self, observation):
        if self.np_random.random() < self.exploration_rate:
            return self.np_random.choice(9)
        ...
        # Copier les Q-values pour ne pas modifier la table originale
        q_values = self.q_table[state].copy()
        
        # Masquer les actions inefficaces (no-go zone)
        # On considère inefficace si efficiency < 0.3 (arbitraire mais exclut la no-go zone ~0.05)
        for action in range(9):
            dir_vec = self._action_to_direction(action)
            eff = calculate_sailing_efficiency(dir_vec, np.array([wx, wy]))
            if eff < 0.3:
