...
class QLearningAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        self.num_angle_bins = 24
        self.velocity_bins = [0.2, 0.5, 1, 2, 5]
        self.wind_bins = [0.5, 1, 2, 5]
        self.goal_dist_bins = [2, 5, 8, 12, 18, 25, 35, 45]
    ...
    def learn(self, state, action, reward, next_state, next_action):
        ...
        # Reward shaping with proximity bonus (AGGRESSIVE)
        shaped_reward = reward - 5  # time penalty
        
        # Distance-based bonus: STRONG encouragement to get close to goal
        # state[2] is g_norm_bin (distance to goal bin)
        g_norm_bin = state[2]
        
        if g_norm_bin == 0:  # Very close to goal (< 5 units)
            shaped_reward += 10.0  # HUGE bonus for being very close!
        elif g_norm_bin == 1:  # Close (5-10 units)
            shaped_reward += 5.0   # Big bonus
        elif g_norm_bin == 2:  # Medium distance (10-20 units)
            shaped_reward += 2.0   # Moderate bonus
        elif g_norm_bin == 3:  # Far (20-30 units)
            shaped_reward += 0.5   # Small bonus
