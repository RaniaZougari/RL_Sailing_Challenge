...
class QLearningAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        # Discount factor (higher = better long-term planning)
    ...
    def learn(self, state, action, reward, next_state, next_action):
        ...
        # Reward shaping
        shaped_reward = reward - 6  # time penalty (higher = encourage shorter paths)
        ...
        # Calculate expected value over all actions weighted by policy probabilities
        ...
        for a in range(9):
            if a == best_action:
                # Probability of selecting best action: (1 - epsilon) + epsilon/9
                prob = (1 - self.exploration_rate) + (self.exploration_rate / 9)
            else:
                # Probability of selecting non-best action: epsilon/9
                prob = self.exploration_rate / 9
            expected_q += prob * q_values[a]
        
        # TD update with expected value
